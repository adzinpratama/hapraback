...
$(function () {
    ...
    let showDetails = $('#show-details'),
        ...
        uniqueShow = $('#unique-show'),
        detailTitle = $('#details-title');
    $(document).on('click', '#post-list li .media-body', function (e) {
        let id = $(this).data('show');
        $.get(route('post.show', { id })).then(res => {
            showDetails.html(res);
            uniqueShow.data('show', id);
            detailTitle.text($(this).data('title'));
            emailDetails.toggleClass('show');
        }).catch(({ responseJSON: er }) => {
            notif('error', { message: er.message });
        })
     ...
     * select single
     * */
    $(document).on('change', '.email-app-list .selectSingle input', function () {
        if ($(this).is(':checked')) {
    ...
    $(document).on('click', '#btn-edit', function (e) {
        e.preventDefault();
        // return console.log(uniqueShow.data('show'));
        return window.location = route('post.edit', { id: uniqueShow.data('show') });
    })
    ...
    $(document).on('click', '#btn-destroy', function (e) {
        e.preventDefault();
        let id = uniqueShow.data('show');
        $.ajax(route('post.destroy', { id }), { method: 'delete' }).then(res => {
